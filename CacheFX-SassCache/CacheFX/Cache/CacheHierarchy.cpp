...
size_t CacheHierarchy::getNInvalidWays () const {
   size_t nInvWays = 0;
  for (int32_t i = 0; i < (int32_t)_cacheHierarchy.size(); i++)
  {
    if (nInvWays < _cacheHierarchy[i]->getNInvalidWays())
    {
      nInvWays = _cacheHierarchy[i]->getNInvalidWays();
    }
  }
  return nInvWays; 
}

int32_t CacheHierarchy::getNPartitions() const {
   size_t nInvWays = 0;
  for (int32_t i = 0; i < (int32_t)_cacheHierarchy.size(); i++)
  {
    if (nInvWays < _cacheHierarchy[i]->getNPartitions())
    {
      nInvWays = _cacheHierarchy[i]->getNPartitions();
    }
  }
  return nInvWays; 
}

bool CacheHierarchy::getLoadBalancing() const {
  bool lb = false;
   for (int32_t i = 0; i < (int32_t)_cacheHierarchy.size(); i++)
  {
    lb = (lb || _cacheHierarchy[i]->getLoadBalancing());
  }
  return lb; 
}
