...
const char* CacheFactory::XMLNODE_CACHE_LOADBALANCING = "LoadBalancing";
const char* CacheFactory::XMLNODE_CACHE_NINVALIDWAYS = "InvalidWays";
...
CacheFactory::CacheFactory()
{
  ...
  repAlgStrMap[REPL_GRAN] = "gran";
  repAlgStrMap[REPL_GLRU] = "glru";
  ...
  strRepAlgMap["gran"] = REPL_GRAN;
  strRepAlgMap["glru"] = REPL_GLRU;
}
                                                 ...
                                                 bool loadBalancing)
{
  if (replAlgorithm != REPL_DIP)
  {
    ...
    else if (description == CustomCache::CACHE_TYPESTR)
    {
      return std::unique_ptr<Cache>(
          new CustomCache(replAlgorithm, nCacheLines / nWays, nWays, nInvalidWays, loadBalancing));
    }
...
void CacheFactory::serializeCache(pugi::xml_node xmlCache, Cache* cache)
{
  ...
  else if (cacheType == CustomCache::CACHE_TYPESTR)
  {
    CustomCache* c = (CustomCache*)cache;
    xmlCache.append_attribute(XMLNODE_CACHE_REPLALG).set_value(repAlgStrMap[c->getAlgorithm()].c_str());
    xmlCache.append_attribute(XMLNODE_CACHE_NLINES).set_value(c->getNSets() * c->getNWays());
    xmlCache.append_attribute(XMLNODE_CACHE_NWAYS).set_value(c->getNWays());
    xmlCache.append_attribute(XMLNODE_CACHE_NPARTITIONS).set_value(c->getNPartitions());
    xmlCache.append_attribute(XMLNODE_CACHE_NINVALIDWAYS).set_value(c->getNInvalidWays());
    xmlCache.append_attribute(XMLNODE_CACHE_LOADBALANCING).set_value(c->getLoadBalancing());
  }
  else if (cacheType == SASSCache::CACHE_TYPESTR) {
    CEASERSCache *c = (CEASERSCache*) cache;
    xmlCache.append_attribute(XMLNODE_CACHE_NLINES).set_value(c->getNSets()*c->getNWays());
    xmlCache.append_attribute(XMLNODE_CACHE_NWAYS).set_value(c->getNWays());
    xmlCache.append_attribute(XMLNODE_CACHE_NPARTITIONS).set_value(c->getNPartitions());
...
std::unique_ptr<Cache> CacheFactory::deserializeCache(pugi::xml_node xmlCache)
{
  ...
  else
  {
    ...
    {
      int32_t nWays = xmlCache.attribute(XMLNODE_CACHE_NWAYS).as_int();
      cache.reset(new CustomCache(
          strRepAlgMap[xmlCache.attribute(XMLNODE_CACHE_REPLALG).as_string()],
          xmlCache.attribute(XMLNODE_CACHE_NLINES).as_int() / nWays, nWays,
          xmlCache.attribute(XMLNODE_CACHE_NPARTITIONS).as_int(),
          xmlCache.attribute(XMLNODE_CACHE_NINVALIDWAYS).as_int(),
          xmlCache.attribute(XMLNODE_CACHE_LOADBALANCING).as_bool()));
    }
    else if (cacheType == SASSCache::CACHE_TYPESTR) {
      int nWays = xmlCache.attribute(XMLNODE_CACHE_NWAYS).as_int();
      cache.reset(new SASSCache(
          xmlCache.attribute(XMLNODE_CACHE_NLINES).as_int()/nWays,
          nWays,
          xmlCache.attribute(XMLNODE_CACHE_NPARTITIONS).as_int()));
    }
  }
