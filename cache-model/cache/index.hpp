...
{
protected:
  const uint32_t imask; // index mask
public:
...
public:
  ...
  virtual int32_t index(uint64_t *latency, uint64_t addr, int32_t skew_idx) {
    latency_acc(latency);
    return (int32_t)((addr >> 6) & imask); // get the set mapping
  }
...
public:
  ...
  virtual int32_t index(uint64_t *latency, uint64_t addr, int32_t skew_idx) {
    ...
    return (int32_t)(hkey ^ oidx); // return the hashed set mapping
  }
...
{
  const uint32_t iwidth;    // index width
...
public:
  ...
  virtual int32_t index(uint64_t *latency, uint64_t addr, int32_t skew_idx) {
    ...
    return (int32_t)((skew_idx << iwidth) + ridx); // return set according to the skew offset + hash index
  }
