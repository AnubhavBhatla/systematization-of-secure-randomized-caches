...
public:
  ...
  std::unordered_map<uint32_t, LocRange> locs; // locs maps from index to a range, most often 0 to numWays - 1
  ...
  void insert(int32_t idx, LocRange r) { locs[idx] = r; } // r is a pair, most often (0,numWays - 1)
  std::string to_string() const;
};
