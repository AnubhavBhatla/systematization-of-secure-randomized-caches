...
{
  for (int32_t i = 0; i < (cacheSize*percentOccupy/100); i += CACHE_LINE_SIZE)
  {
    read(evictionSet, i);
  }
}
...
{
  int32_t result = 0;
  for (int32_t i = CACHE_LINE_SIZE; i <= (cacheSize*percentOccupy/100); i += CACHE_LINE_SIZE)
  {
    if (read(evictionSet, (cacheSize*percentOccupy/100) - i) == 0)
      result++;
  }
...
bool OccupancyAttacker::warmup()
{
  ...
  { 
    ...
    {
      if (read(warmupSet, cacheSize - i) == 0)
        p++;
    }
