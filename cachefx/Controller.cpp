...
Controller::Controller(CLArgs& args)
{
  ...
  int32_t nsets = cache.get()->getNSets();
  if (strcmp(cache.get()->getCacheType(),"sasscache") != 0) {    
    int32_t ninvalidways = cache.get()->getNInvalidWays();
    int32_t npartitions = cache.get()->getNPartitions();
    nlines = nlines - npartitions*nsets*ninvalidways;
  }
  // std::cout<<cache.get()->getCacheType()<<"\n";
  // std::cout<<ninvalidways<<" "<<npartitions<<" "<<nsets<<"\n";
  // std::cout<<(nlines - npartitions*nsets*ninvalidways)<<"\n";
  // exit(0);

  // std::cout<<args.percentOccupy<<"\n";

  // nlines = ((args.percentOccupy*nlines)/100);
  // std::cout<<nlines<<std::endl;
  // nlines = 256;
  // std::cout << npartitions << nsets << ninvalidways << nlines << std::endl;
